...
type MappedRestaurant = {
  id: string; name: string; cuisine: string; location: string;
  description: string; priceLevel: string; imageUrl: string;
  vibe: { intimacy: number; energy: number; formality: number; noise: number; outdoorsy: number; };
  averageRating: number; reviewCount: number; matchScore: number;
};

const moodChips = Object.keys(moodProfiles);

export default function HomePageClient({ dbRestaurants }: { dbRestaurants: DbRes[] }) {
  ...
  const scoredRestaurants = useMemo(() => {
    ...
    // Convert DB restaurants to match the shape expected by our UI
    const allRestaurants = dbRestaurants.map(r => ({
      id: r.id,
      name: r.name,
      cuisine: r.cuisine,
    ...
    if (!profile) return allRestaurants.map(r => ({ ...r, matchScore: 0 }));
      ...
      <section className="bg-cream pt-24 pb-16 px-6 sm:px-12 lg:px-24 border-b border-rust/10 w-full">
        <div className="max-w-4xl mx-auto text-center space-y-8">
          <h1 className="font-cormorant text-5xl sm:text-6xl md:text-7xl font-semibold text-ink leading-tight">
            Find the perfect spot for <span className="italic text-rust">your mood</span>
          </h1>
          <p className="text-lg text-sage font-light max-w-2xl mx-auto">
            Not just what's nearby. Discover restaurants that match the exact vibe you're looking for right now.
          </p>
          ...
          <div className="relative max-w-2xl mx-auto mt-8">
            ...
            <input
              type="text"
              className="w-full pl-12 pr-4 py-4 rounded-full border border-rust/20 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-transparent text-ink placeholder:text-sage/60 transition-all font-outfit"
              placeholder="e.g., somewhere cozy for a date tonight, not too loud..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <button className="absolute inset-y-2 right-2 bg-rust text-cream px-6 rounded-full font-medium hover:bg-rust/90 transition-colors">
              Find Vibe
            </button>
          </div>

          <div className="pt-8">
            <p className="text-sm text-sage/80 mb-4 uppercase tracking-widest font-dm-mono">Or quick select</p>
          ...
          <div className="flex justify-between items-end mb-8">
            <div>
              <h2 className="font-cormorant text-3xl font-semibold text-ink">
                Perfect for <span className="italic text-rust">{selectedMood}</span>
              </h2>
              <p className="text-sage mt-2">Showing {scoredRestaurants.length} tailored recommendations</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {scoredRestaurants.map((restaurant) => (
...
function RestaurantCard({ restaurant }: { restaurant: MappedRestaurant }) {
  return (
    <Link href={`/restaurant/${restaurant.id}`} className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-400 border border-rust/5 hover:-translate-y-1">
      <div className="relative h-60 overflow-hidden bg-gray-100">
        ...
        <button className="absolute top-4 right-4 p-2 rounded-full bg-white/50 backdrop-blur-md text-ink hover:bg-white transition-colors">
          <Heart className="w-5 h-5" />
        </button>
      </div>
      
      <div className="p-6 flex flex-col flex-1">
        <h3 className="font-cormorant text-2xl font-semibold text-ink leading-tight mb-1">{restaurant.name}</h3>

        <div className="flex items-center gap-1 mb-3">
          <Star className="w-3.5 h-3.5 text-gold fill-gold" />
          <span className="text-xs font-medium text-ink">{restaurant.averageRating.toFixed(1)}</span>
        ...
        <p className="text-ink/70 text-sm mb-6 line-clamp-3 leading-relaxed flex-1">
